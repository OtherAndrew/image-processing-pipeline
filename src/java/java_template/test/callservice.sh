...
time output=`aws lambda invoke --invocation-type RequestResponse --cli-binary-format raw-in-base64-out --function-name resizeImage --region us-east-1 --payload $json /dev/stdout | head -n 1 | head -c -2 ; echo`
echo
echo "JSON RESULT:"
echo $output | jq
echo
